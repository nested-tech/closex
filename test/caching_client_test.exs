defmodule Closex.CachingClientTest do
  ...
  describe "update_lead/2" do
    ...
    test "invalidates cache when updating lead" do
      ...
      {:ok, updated_lead} =
        Closex.CachingClient.update_lead(@update_lead_id, %{"name" => "New Name"})

      assert_received {:closex_mock_client, :update_lead,
                       [@update_lead_id, %{"name" => "New Name"}, []]}
  ...
  describe "update_opportunity/2" do
    ...
    test "invalidates cache when updating opportunity" do
      ...
      {:ok, updated_opportunity} =
        Closex.CachingClient.update_opportunity(@update_opportunity_id, %{"confidence" => 50})

      assert_received {:closex_mock_client, :update_opportunity,
                       [@update_opportunity_id, %{"confidence" => 50}, []]}

      assert updated_opportunity["id"] == @update_opportunity_id
      assert updated_opportunity["confidence"] == 50

      assert Closex.CachingClient.get_opportunity(@update_opportunity_id) ==
               {:ok, updated_opportunity}
    end
  end
